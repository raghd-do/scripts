...
database = load_workbook("database\قاعدة الفصل الأول عام 1444 - بنات.xlsx")
...
for test in tests:
    ...
    # talqeen = database["منهج التلقين"]

    found = False
    # التلقين
    # for row in talqeen.iter_rows(min_row=2, max_row=len(list(talqeen.rows))):
    #     if row[3].value == student_name and row[4].value == student_id:
    #         print("we found her :D")
    #         found = True

    #         print("now we'r copying her marks")
    #         talqeen.cell(row=row[1].row, column=13).value = sheet.cell(
    #             row=14, column=6).value

    #         print("copying is done :)")
    #         database.save(
    #             filename="database\قاعدة الفصل الأول عام 1444 - بنات.xlsx")
